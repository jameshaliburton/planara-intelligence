...
type Stage = { name: string; sub: string };
type Handoff = {
  from: string;
  to: string;
  friction: string;
...
const factoryStages: Stage[] = [
  { name: "Corporate Manufacturing", sub: "Engineering" },
  { name: "Plant", sub: "Site Operations" },
  { name: "Operator", sub: "Shop Floor" },
];

const factoryHandoffs: Handoff[] = [
  {
    from: "Corporate Manufacturing",
    to: "Plant",
    friction: "Documentation distributed as PDFs to site engineers",
    opportunity: "Queryable TSI platform shared across plants",
  },
  {
    from: "Plant",
    to: "Operator",
    friction: "Printed SOPs and tribal knowledge",
    opportunity:
      "Structured procedures with safety warnings on shop-floor tablets",
  },
];
  ...
  return (
    <div className="flex-1 min-w-[40px] flex items-center px-1">
      ...
      >
        ...
        />
        <polygon points="92,0 100,1 92,2" fill="rgba(67,206,214,0.25)" />
      </svg>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="hidden md:block px-8">
        <p className="text-[10px] font-mono uppercase tracking-wider text-white/30 mb-4">
          {label}
        </p>
          ...
          {stages.map((stage, i) => (
            <div key={stage.name} className="contents">
              <div className="border border-white/10 rounded-lg px-5 py-4 text-center bg-white/[0.02] min-w-[110px] shrink-0">
                ...
                <p className="text-[10px] text-white/30 mt-0.5">{stage.sub}</p>
              </div>
        ...
        <div className={`grid ${gridCols} gap-6 mt-8 ml-[55px] mr-[55px]`}>
          {handoffs.map((h, i) => (
...
export function ValueChainDiagram() {
  return (
    <ScrollReveal>
      <div className="my-12 py-8 border border-white/[0.06] rounded-lg bg-white/[0.015] space-y-10">
        <Shape
          label="Classic equipment-industry shape"
          stages={classicStages}
          handoffs={classicHandoffs}
          footer="Owner &rarr; OEM: every interaction becomes fleet intelligence"
        ...
        <Shape
          label="Factory-floor shape"
          stages={factoryStages}
          handoffs={factoryHandoffs}
